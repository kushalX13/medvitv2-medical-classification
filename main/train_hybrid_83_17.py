...
def build_hybrid_dataset(czech_path, bloodmnist_npz_path):
    ...
    czech_len = int(0.83 * len(czech_dataset))
    blood_len = int(0.17 * len(blood_dataset))
...
def train_model(model, train_loader, val_loader, device, save_path, plot_dir, epochs, lr):
    ...
    plt.savefig(os.path.join(plot_dir, "accuracy_curve_83_17.png"))
    plt.close()
    ...
    plt.savefig(os.path.join(plot_dir, "loss_curve_83_17.png"))
    plt.close()
...
def evaluate(model, dataset, name, plot_dir, device):
    ...
    plt.savefig(os.path.join(plot_dir, f"confusion_matrix_{name}_83_17.png"))
    plt.close()
    ...
    with open(os.path.join(plot_dir, f"metrics_{name}_83_17.txt"), "w") as f:
